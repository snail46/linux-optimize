...
#  功能：TCP调优 / 按内存&CPU动态分级 / 安全边界保护（不含DNS修改）
...
if $REVERT; then
    step "还原优化配置"
    # 使用最早的备份目录:多次运行脚本后,较新的备份保存的是上一次
    # 优化后的配置而非系统原始状态,只有首次运行前的备份才是原貌。
    LATEST_BACKUP=$(ls -td /etc/network-optimize-backup-* 2>/dev/null | tail -1 || true)
    [[ -z "$LATEST_BACKUP" ]] && die "未找到备份目录，无法还原"
    info "使用最早备份目录（系统原始状态）: $LATEST_BACKUP"
    ...
    run "systemctl disable disable-thp.service 2>/dev/null || true"
    run "rm -f /etc/systemd/system/disable-thp.service"
...
if (( MEM_TOTAL_MB < 1536 )) && (( SWAP_TOTAL_KB == 0 )); then
    ...
    if (( SWAP_SIZE_MB < MIN_USEFUL_SWAP_MB )); then
        ...
        if $AUTO_YES; then
            ...
            # 只在 stdin 是终端时才交互询问。
            # 通过 curl|bash 等管道方式运行时 stdin 非终端,read 会失败并
            # 因 set -e 直接终止整个脚本(后续优化全部不执行)。
            # 非交互环境下默认跳过 swap 创建,需要时用 --yes 显式开启。
            if [[ -t 0 ]]; then
                read -rp "是否自动创建 ${SWAP_SIZE_MB}MB Swap 文件以提升稳定性？[Y/n]: " ans || ans="n"
                [[ -z "$ans" || "$ans" =~ ^[Yy]$ ]] && NEED_SWAP=true
            else
                info "非交互式运行,跳过 Swap 创建询问(如需自动创建请加 --yes 参数)"
            fi
        fi
    fi
...
# ── 端口与快速重用 ────────────────────────────────────────────
# 临时端口范围保持内核默认（32768-60999），不做扩大。
# 原因：扩大到 1024-65535 会让系统出站连接随机占用面板/中转工具
# 分配给用户的业务端口，导致新规则 bind 失败（间歇性、难排查）。
# 配合 tcp_tw_reuse=1，默认 2.8 万个端口对代理节点完全够用。
# 若确实遇到端口耗尽，正确做法是先用 net.ipv4.ip_local_reserved_ports
# 保留业务端口段，再按需扩大范围。
...
# 注：不设置 vm.overcommit_memory=1（那是 Redis/fork 密集型应用的配置）。
# 代理服务场景保持内核默认启发式模式，内存吃紧时应用能收到分配失败
# 并优雅处理，而不是被 OOM killer 随机强杀，对小内存机器更安全。
"
...
#  Step 6: DNS 优化
# ══════════════════════════════════════════════════════════════════════════════
step "DNS 状态检查（仅展示，不修改）"

HAS_RESOLVED=$(systemctl is-active systemd-resolved 2>/dev/null || echo "inactive")
info "systemd-resolved: $HAS_RESOLVED"
info "本脚本不再修改 DNS 配置（此前的 DNS 优化在部分海外节点上导致间歇性解析失败，已移除）"
info "当前 DNS 配置如下，如需调整请手动处理："
if [[ -f /etc/resolv.conf ]]; then
    cat /etc/resolv.conf | sed 's/^/    /' | tee -a "$LOG_FILE"
fi
...
if [[ -z "$PRIMARY_IF" ]]; then
    ...
    if command -v ethtool &>/dev/null; then
        if ! $DRY_RUN; then
            # offload(GRO/TSO/GSO)策略：
            #   仅在裸金属上显式开启；虚拟化环境保持驱动默认值。
            #   原因：virtio等虚拟网卡驱动默认已启用合适的offload，
            #   强制开启在部分 虚拟网卡+NAT/conntrack 组合下有已知的
            #   间歇性丢包/连接重置问题，且极难排查。
            if [[ "$VIRT" == "none" ]]; then
                ethtool -K "$PRIMARY_IF" gro on tso on gso on >> "$LOG_FILE" 2>&1 && \
                    log "裸金属环境：已开启网卡 offload (gro/tso/gso)" || true
            else
                info "虚拟化环境（$VIRT）：保持网卡 offload 驱动默认值，不强制修改"
            fi
    ...
    if (( CPU_CORES >= 2 && CPU_CORES <= 32 )) && ! $DRY_RUN; then
        # RPS 掩码用位运算生成,bash 为 64 位运算且 sysfs 掩码超过 32 位
        # 需要逗号分隔格式,因此仅在 2~32 核范围内手动配置 RPS;
        # 更多核的机器交给 irqbalance + 网卡多队列,效果更好且无格式问题。
        CPU_MASK=$(printf '%x' $(( (1 << CPU_CORES) - 1 )))
        RPS_QUEUES=0
        for rxq in /sys/class/net/"$PRIMARY_IF"/queues/rx-*/rps_cpus; do
            [[ -f "$rxq" ]] || continue
            echo "$CPU_MASK" > "$rxq" 2>/dev/null && RPS_QUEUES=$((RPS_QUEUES+1)) || true
        done
        if (( RPS_QUEUES > 0 )); then
            # RFS 需要全局表和每队列 flow_cnt 两者都配置才会生效
            echo 32768 > /proc/sys/net/core/rps_sock_flow_entries 2>/dev/null || true
            FLOW_CNT=$(( 32768 / RPS_QUEUES ))
            for rxq in /sys/class/net/"$PRIMARY_IF"/queues/rx-*/rps_flow_cnt; do
                [[ -f "$rxq" ]] || continue
                echo "$FLOW_CNT" > "$rxq" 2>/dev/null || true
            done
            log "多核机器已启用 RPS+RFS（${RPS_QUEUES}个接收队列,flow_cnt=${FLOW_CNT}）"
        fi
    ...
    elif (( CPU_CORES > 32 )) && ! $DRY_RUN; then
        info "核数较多（${CPU_CORES}核）,跳过手动 RPS,由 irqbalance + 网卡多队列处理"
        if ! command -v irqbalance &>/dev/null; then
            if command -v apt-get &>/dev/null; then
                run "apt-get install -y irqbalance"
            elif command -v yum &>/dev/null; then
...
if [[ -f "$THP_PATH" ]]; then
    ...
    if ! $DRY_RUN; then
        ...
        # 持久化:优先使用 systemd oneshot unit。
        # 不用 rc.local:Debian 11+/Ubuntu 22+ 默认不启用 rc-local.service,
        # 写进去的命令重启后根本不执行,持久化会静默失效。
...
UNIT
            ...
            systemctl enable disable-thp.service >> "$LOG_FILE" 2>&1 && \
                log "THP 已禁用（systemd unit 持久化,重启后自动生效）" || \
                warn "THP systemd unit 启用失败,重启后 THP 可能恢复默认"
        else
            # 无 systemd 的环境降级到 rc.local
            ...
            log "THP 已禁用（rc.local 持久化,请确认该机制在本系统可用）"
        fi
    fi
else
